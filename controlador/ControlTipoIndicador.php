...
class ControlTipoIndicador
{
    ...
    function listarIndicadorTipoIndicador($fkIdTipoIndicador)
    {
        $comandoSql = "SELECT indicador.fkidtipoindicador,tipoindicador.nombre 
            FROM indicador INNER JOIN tipoindicador ON indicador.fkidtipoindicador = tipoindicador.id
            WHERE fkidtipoindicador = '$fkIdTipoIndicador'";
        $objControlConexion = new ControlConexion();
        $objControlConexion->abrirBd(
            $GLOBALS['serv'],
            $GLOBALS['usua'],
